...
- **[claude-rag-hook](https://github.com/ra-yavuz/claude-rag-hook)** &nbsp; CLI + Claude Code hook. Type `rag: <question>` in Claude Code; a `UserPromptSubmit` hook walks up from cwd, finds a per-folder LanceDB index, embeds the query, retrieves the top-K chunks, and prepends them to the prompt before Claude sees it. Keyword-triggered (not MCP), so the model never has to decide when to retrieve, and prompts that do not start with the trigger pass through with zero token overhead. Pluggable embedders (fastembed, OpenAI-compatible, hydra-llm interop) and a small per-user warm daemon for sub-second retrieval.
