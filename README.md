...
- **[claude-can-speak](https://github.com/ra-yavuz/claude-can-speak)** &nbsp; npm CLI + Claude Code hook & skill. Speech-out for Claude Code, the companion to the built-in `/voice` speech-in: turn `/voice` on and Claude reads its replies aloud, turn it off and you are silent again. Two modes from one package: a Stop hook that speaks every finished reply, or a `speak` skill that lets Claude voice only deliberate notifications (a spoken "build is done", a heads-up while you look away). Local neural text-to-speech runs in Docker, Kokoro for natural English and Piper for German, Turkish, and more, so nothing goes to the cloud and there are no API keys. Interrupt any time with `claude-can-speak stop` or just by sending your next message. `npm install -g claude-can-speak`.
