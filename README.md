...
- **[chiron](https://github.com/ra-yavuz/chiron)** &nbsp; CLI + agent hooks. A mentor kit for coding agents: when you fall back from a top-tier model to a weaker one, the weaker one inherits the strong one's ways of working. An operating doctrine (verify before acting, no workarounds, honest completion claims, push back on bad requests) plus mechanical enforcement hooks for Claude Code that deny `--no-verify`, error swallowing, force pushes, and test deletion outright, block unverified "done" claims, and inject the project's real file inventory so paths do not get hallucinated. Codex CLI gets the doctrine via `AGENTS.md`. Ships an A/B eval harness with pre-registered checks that measures whether the kit actually moves model behavior, and publishes the numbers either way. Install by pointing your agent at the repo's `INSTALL-PROMPT.md`; it clones, wires, and self-tests. Grew out of the doctrine-plus-hooks pattern documented in `vigil`.
...
- **[local-jev](https://github.com/ra-yavuz/local-jev)** &nbsp; CLI + local API. Run a Jev-style typed-decision API locally over GGUF models. `local-jev setup` creates an isolated runtime and downloads an open model, then `local-jev serve` exposes `/v1/systemone` for yes/no, choice, and score questions. Instead of asking the model to generate JSON, it reads the next-token logits for fixed answer letters and returns structured probabilities. Includes a helper script to compare the same request against Kev.
...
One line. Sets up the signed repo if not already added, refreshes the package index, and installs the package. Replace `<package>` with `inhibit-charge`, `herald`, `meowtrics`, `lillycoder`, `hydra-llm`, `hydracoder`, `hydra-rag-hooks`, `portunus-ovpn`, `edgecall`, or `local-jev`:
