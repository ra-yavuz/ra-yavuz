...
- **[doublethink](https://github.com/ra-yavuz/doublethink)** &nbsp; CLI + broker. ntfy you can actually trust: a publish/subscribe message broker as easy to stand up as ntfy, but with real authentication and genuinely private channels. ntfy is effortless because its topic name is the only gate, which is also why its topics are effectively public. doublethink keeps the minutes-to-set-up ergonomics and adds identity and private channels: a private channel admits only authenticated, authorised parties (Ed25519 challenge, no reusable token), and its payloads are end-to-end encrypted between the paired peers, so the broker operator never sees the plaintext. Knowing a channel name grants no access, and pairing is man-in-the-middle resistant (single-use codes plus an out-of-band short authentication string). Bidirectional, asynchronous, streaming, with opt-in plaintext public topics too. Cross-platform: run it in Docker or as a single native binary (Go).
